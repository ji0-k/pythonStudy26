class Board:
    def __init__(self, id, title, content, member_id, active=True, created_at=None,writer_name=None, writer_uid=None):
        self.id = id  # DB의 PK
        self.title = title
    ...
    @classmethod
    def from_db(cls, row: dict): #객체 한줄로 가져와 클래스만들기
        if not row: return None
        #DB에있는 내용에 한줄을 dict차입으로 가져와 객체로 만든다
        return cls(
            id=row.get('id'),
            title=row.get('title'),
